...
static void ota_start_write_flash()
{
    ...
    for (sec_idx = 0; sec_idx < max_idx; sec_idx++)
    {
        ...
        s907x_hal_flash_erase_resume_tick(EraseSector, flash_addr);
    }
}
...
static int ota_write_flash(ota_flash_partition_t * pst_ota_flash, char *flash_buf, char *data, int data_len)
{
    ...
    if (wr_len <= data_len) {
        ...
        s907x_hal_flash_write_resume_tick(flash_addr, (u8 *)flash_buf, SECTOR_SIZE);
...
void ota_init_flash_params(void)
{
    ...
    s907x_flash_read(FLASH_PARAMETER_1, (u8 *)&ota_param, sizeof(ota_param));
    if (FLASH_OTA1 - FLASH_ADDR_BASE == ota_param.orig_addr) {
        ...
        s907x_flash_write(FLASH_PARAMETER_1, (u8 *)&ota_param, sizeof(ota_param));
...
int ota_write_boot_params(void *ota_header, int rx_bin_len)
{
    ...
    s907x_flash_write(FLASH_PARAMETER_1, (u8 *)&ota_param, sizeof(ota_param));
    s907x_flash_read(FLASH_PARAMETER_1, (u8 *)&ota_param_r, sizeof(ota_param_r));
...
void ota_init_boot_params()
{
    ...
    s907x_flash_read(FLASH_PARAMETER_1, (u8 *)&boot_param, sizeof(boot_param));
    ...
    s907x_flash_write(FLASH_PARAMETER_1, (u8 *)&boot_param, sizeof(boot_param));
    s907x_flash_read(FLASH_PARAMETER_1, (u8 *)&boot_param_r, sizeof(boot_param_r));
