...
/* 
AT+CWMODE_CUR set Wi-Fi mode (STA/AP/STA+AP), WILL NOT SAVE Flash
AT+CWMODE_DEF set Wi-Fi mode (STA/AP/STA+AP), SAVE Flash
AT+CWJAP_CUR connect AP, WILL NOT SAVE Flash
AT+CWJAP_DEF connect AP, SAVE Flash
AT+CWLAPOPT set properties of AT+CWLAPinstruction scan results
AT+CWLAP scan nearby AP to get information
AT+CWQAP disconnect with AP
AT+CWSAP_CUR set s907x SoftAP configuration, WILL NOT SAVE Flash
AT+CWSAP_DEF set s907x SoftAP mode (STA/AP/STA+AP), SAVE Flash
AT+CWLIF get infomation of station connectted to s907x SoftAP
AT+CWDHCP_CUR set DHCP, WILL NOT SAVE Flash
AT+CWDHCP_DEF set DHCP, SAVE Flash
AT+CWDHCPS_CUR set s907x SoftAP DHCP IP range, WILL NOT SAVE Flash
AT+CWDHCPS_DEF set s907x SoftAP DHCP IP range, SAVE Flash
AT+CWAUTOCONN set a flag if auto-connect AP when power on
AT+CIPSTAMAC_CUR set s907x MAC address of Station, WILL NOT SAVE Flash
AT+CIPSTAMAC_DEF set s907x MAC address of Station, SAVE Flash
AT+CIPAPMAC_CUR set s907x MAC address of SoftAP, WILL NOT SAVE Flash
AT+CIPAPMAC_DEF set s907x MAC address of SoftAP, SAVE Flash
AT+CIPSTA_CUR set s907x IP address of Station, WILL NOT SAVE Flash
AT+CIPSTA_DEF set s907x IP address o Station, SAVE Flash
AT+CIPAP_CUR set s907x IP address o SoftAP, WILL NOT SAVE Flash
AT+CIPAP_DEF set s907x IP address o SoftAP, SAVE Flash
AT+CWSTARTSMART start SmartConfig
AT+CWSTOPSMART stop SmartConfig
AT+CWSTARTDISCOVER Turn on the mode that can be detected by wechat in WLAN
AT+CWSTOPDISCOVER Turn off the mode that can be detected by wechat in LAN
AT+WPS set WPS function
AT+MDNS set MDNS function
AT+CWHOSTNAME set hostname of s907x Station
AT+CWCOUNTRY_CUR set s907x current WiFi country code
AT+CWCOUNTRY_DEF set s907x default WiFi country code
*/
...
void wlan_flash_init(void)
{
    ...
    at_wlan_flash_write_kv(WIFI_OPMODE_ADDR,            &mode,                      WIFI_OPMODE_SIZE);
    at_wlan_flash_write_kv(WIFI_SAP_DEF_ADDR,           (u8*)&g_atwlan_cur_ap_cfg,  WIFI_SAP_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_DHCP_DEF_ADDR,          &g_atwlan_dhcp_stat,        WIFI_DHCP_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_DHCPS_DEF_ADDR,         (u8*)&dhcps_lease,          WIFI_DHCPS_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_RECON_ADDR,             &reconnect_mark,            WIFI_RECON_SIZE);
    at_wlan_flash_write_kv(WIFI_STA_MAC_DEF_ADDR,       sta_mac,                    WIFI_STA_MAC_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_AP_MAC_DEF_ADDR,        ap_mac,                     WIFI_AP_MAC_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_STA_IP_ADDR_DEF_ADDR,   (u8*)&g_atwlan_sta_if,      WIFI_STA_IP_ADDR_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_AP_IP_ADDR_DEF_ADDR,    (u8*)&g_atwlan_ap_if,       WIFI_AP_IP_ADDR_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_COUNTRY_DEF_ADDR,       &country_code,              WIFI_COUNTRY_DEF_SIZE);
...
void at_wlan_init_def_para()
{
    ...
    at_wlan_flash_read_kv(WIFI_OPMODE_ADDR,             &mode,                      WIFI_OPMODE_SIZE);
    ...
    at_wlan_flash_read_kv(WIFI_OPMODE_ADDR,             &mode,                      WIFI_OPMODE_SIZE);
    at_wlan_flash_read_kv(WIFI_SAP_DEF_ADDR,            (u8*)&g_atwlan_cur_ap_cfg,  WIFI_SAP_DEF_SIZE);
    at_wlan_flash_read_kv(WIFI_DHCP_DEF_ADDR,           &g_atwlan_dhcp_stat,        WIFI_DHCP_DEF_SIZE);
    at_wlan_flash_read_kv(WIFI_DHCPS_DEF_ADDR,          (u8*)&g_atwlan_dhcps_lease, WIFI_DHCPS_DEF_SIZE);
    at_wlan_flash_read_kv(WIFI_STA_IP_ADDR_DEF_ADDR,    (u8*)&g_atwlan_sta_if,      WIFI_STA_IP_ADDR_DEF_SIZE);
    at_wlan_flash_read_kv(WIFI_AP_IP_ADDR_DEF_ADDR,     (u8*)&g_atwlan_ap_if,       WIFI_AP_IP_ADDR_DEF_SIZE);
...
void atwlan_init_mac(u8 mode)
{
	...
	if(mode & S907X_MODE_STA){
		...
		at_wlan_flash_read_kv(WIFI_STA_MAC_DEF_ADDR, mac, WIFI_STA_MAC_DEF_SIZE);
		s907x_wlan_set_mac_address_no_efuse(if_idx,mac);
	}

	if(mode & S907X_MODE_AP){
		if_idx=at_wlan_get_if_idx(AT_WLAN_AP);
		at_wlan_flash_read_kv(WIFI_AP_MAC_DEF_ADDR, mac, WIFI_AP_MAC_DEF_SIZE);
		s907x_wlan_set_mac_address_no_efuse(if_idx,mac);
	}
}
...
err:
	memset(&sta_cfg,0,sizeof(atwlan_sta_cfg_def_t));
	at_wlan_flash_write_kv(WIFI_JAP_DEF_ADDR, (u8*)&sta_cfg, WIFI_JAP_DEF_SIZE);
    at_rsp(AT_ERROR);
    return;    
save_to_flash:    
#if WLAN_PARA_SAVE
...
static void atwlan_cwsap_def (void *context)
{
    ...
    if(!cmd->setsize && cmd->mode == AT_MODE_R){
        #if WLAN_PARA_SAVE
        at_wlan_flash_read_kv(WIFI_SAP_DEF_ADDR, (u8*)&sap_cfg, WIFI_SAP_DEF_SIZE);
        #endif
        at_rsp(AT_WLAN_SAP_CUR, 
            sap_cfg.ssid, 
            sap_cfg.password, 
...
#if WLAN_PARA_SAVE    
    sap_cfg.max_conn = AT_WLAN_MAX_STA;
    at_wlan_flash_write_kv(WIFI_SAP_DEF_ADDR, (u8*)&sap_cfg, WIFI_SAP_DEF_SIZE);
#endif
...
static void atwlan_cwdhcp_def (void *context)
{
    ...
    if(!cmd->setsize && cmd->mode == AT_MODE_R){
        #if WLAN_PARA_SAVE
        at_wlan_flash_read_kv(WIFI_DHCP_DEF_ADDR, &dhcp_stat, WIFI_DHCP_DEF_SIZE);
        #endif
...
static void atwlan_cwdhcps_def (void *context)
{
    ...
    if(!cmd->setsize && cmd->mode == AT_MODE_R){
    #if WLAN_PARA_SAVE
        at_wlan_flash_read_kv(WIFI_DHCPS_DEF_ADDR, (u8*)&dhcps_lease, WIFI_DHCPS_DEF_SIZE);
    #endif
...
save_to_flash:        
    ...
    at_wlan_flash_write_kv(WIFI_RECON_ADDR, &enable_flash, WIFI_RECON_SIZE);        
#endif
    at_rsp(AT_OK);
}     
...
save_to_flash:    
#if WLAN_PARA_SAVE
    at_wlan_flash_write_kv(WIFI_STA_MAC_DEF_ADDR, mac, WIFI_STA_MAC_DEF_SIZE);     
#endif
...
save_to_flash: 
#if WLAN_PARA_SAVE
    at_wlan_flash_write_kv(WIFI_AP_MAC_DEF_ADDR, mac, WIFI_AP_MAC_DEF_SIZE);        
#endif
...
static void atwlan_cipsta_def (void *context)
{
    ...
    if(!cmd->setsize && cmd->mode == AT_MODE_R){
    #if WLAN_PARA_SAVE
        at_wlan_flash_read_kv(WIFI_STA_IP_ADDR_DEF_ADDR, (u8*)&if_addr, WIFI_STA_IP_ADDR_DEF_SIZE);
    #endif
...
save_to_flash:
#if WLAN_PARA_SAVE
    at_wlan_flash_write_kv(WIFI_STA_IP_ADDR_DEF_ADDR, (u8*)&if_addr, WIFI_STA_IP_ADDR_DEF_SIZE);
    at_wlan_flash_write_kv(WIFI_DHCP_DEF_ADDR, &dhcp_stat, WIFI_DHCP_DEF_SIZE);
#endif
...
static void atwlan_cipap_def (void *context)
{
    ...
    if(!cmd->setsize && cmd->mode == AT_MODE_R){
    #if WLAN_PARA_SAVE
        at_wlan_flash_read_kv(WIFI_AP_IP_ADDR_DEF_ADDR, (u8*)&orig_if_addr, WIFI_AP_IP_ADDR_DEF_SIZE);
    #endif
...
save_to_flash:    
#if WLAN_PARA_SAVE
    at_wlan_flash_write_kv(WIFI_AP_IP_ADDR_DEF_ADDR, (u8*)&new_if_addr, WIFI_AP_IP_ADDR_DEF_SIZE);
#endif    
...
//AT+CWSTOPDISCOVER Turn off the mode that can be detected by wechat in WLAN  
...
static void atwlan_cwcountry_def  (void *context)
{
    ...
    if(!cmd->setsize && cmd->mode == AT_MODE_R){
    #if WLAN_PARA_SAVE
        at_wlan_flash_read_kv(WIFI_COUNTRY_DEF_ADDR, (u8*)&country_code, WIFI_COUNTRY_DEF_SIZE);    
    #endif
        if(country_code == S907X_COUNTRY_CN)
            strcpy(country,"CN");
        else if(country_code == S907X_COUNTRY_US)
...
save_to_flash:        
#if WLAN_PARA_SAVE
    at_wlan_flash_write_kv(WIFI_COUNTRY_DEF_ADDR, (u8*)&country_code, WIFI_COUNTRY_DEF_SIZE);    
#endif
